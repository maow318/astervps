  ...
  var body: some View {
    ZStack {
      ...
      Circle().trim(from: 0, to: progress).stroke(
        AngularGradient(
          colors: [tint.opacity(0.55), tint, AsterColor.chartPalette[4]], center: .center),
...
#Preview {
  MetricRing(value: 63, label: L.text("metric.cpu")).padding().background(AsterColor.background1)
}
