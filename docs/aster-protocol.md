...
`401 {"error":"unauthorized"}` indicates a missing or incorrect bearer token. Invalid history parameters return `404 {"error":"invalid since"}`.

## Security

The agent creates an ECDSA P-256 self-signed certificate when no paths are supplied and prints its DER SHA-256 fingerprint. Clients must use TOFU: show this fingerprint on first connection, store the accepted value for that machine, and reject future certificate changes. A bearer token remains required; TLS protects it only in transit.
