        ...
        VStack(alignment: .leading, spacing: AsterSpacing.sm) {
          HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 3) {
              ...
              Label(node.info.region, systemImage: "location.fill").font(AsterTypography.caption)
          ...
          HStack(spacing: AsterSpacing.md) {
            ...
            VStack(alignment: .trailing, spacing: 5) {
              MetricLabel(
                symbol: "arrow.down",
                value: String(format: "%.1f", node.metrics.downloadBytesPerSecond / 1_000_000),
                unit: "MB/s", tint: AsterColor.chartPalette[1])
              MetricLabel(
                symbol: "arrow.up",
                value: String(format: "%.1f", node.metrics.uploadBytesPerSecond / 1_000_000),
                unit: "MB/s", tint: AsterColor.chartPalette[4])
            }
          }
          ...
          HStack {
            Label(
              Duration.seconds(node.metrics.uptime).formatted(
                .units(allowed: [.days, .hours], width: .abbreviated)), systemImage: "clock"
            ).font(AsterTypography.caption).foregroundStyle(AsterColor.foregroundSecondary)
            Spacer()
