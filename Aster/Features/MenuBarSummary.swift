...
struct MenuBarSummary: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 6) {
      ...
      ForEach(store.nodes.prefix(5)) { node in
        HStack(spacing: 8) {
          StatusDot(status: node.info.status, diameter: 7)
          Text(node.info.name).lineLimit(1)
          Spacer()
          MiniBar(value: node.metrics.cpuUsage, tint: AsterColor.chartPalette[0])
          MiniBar(value: node.metrics.memoryUsage, tint: AsterColor.chartPalette[1])
...
private struct MiniBar: View {
  ...
  let tint: Color
  var body: some View {
    Capsule().fill(tint.opacity(0.2)).overlay(alignment: .leading) {
      Capsule().fill(tint).frame(width: 34 * value / 100)
