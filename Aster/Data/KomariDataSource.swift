...
final class KomariDataSource: MonitorDataSource {
  ...
  func loadHistory(for node: NodeInfo, hours: Int) async throws -> MetricsHistory {
    ...
    let response: KomariEnvelope<KomariHistoryPayload> = try await request(
      ...
      query: [
        URLQueryItem(name: "uuid", value: serverID), URLQueryItem(name: "hours", value: "\(hours)"),
      ]
    )
  ...
  private func openSocket() {
    ...
    guard
      var components = URLComponents(
        url: baseURL.appendingPathComponent("api/clients"), resolvingAgainstBaseURL: false)
    else { return }
  ...
  private func applyRealtimePayload(_ data: Data) {
    guard let payload = try? JSONDecoder.komari.decode(KomariSocketEnvelope.self, from: data) else {
      return
    }
  ...
  private func makeInfo(_ node: KomariNode) -> NodeInfo {
    NodeInfo(
      id: UUID(uuidString: node.uuid) ?? UUID(), name: node.name,
      region: node.group ?? node.region ?? "", flag: node.region?.containsEmoji ?? "",
      operatingSystem: node.os ?? "", status: .offline,
      tags: node.tags?.split(separator: ";").map(String.init) ?? [], groupID: nil,
      createdAt: node.createdAt ?? .now, serverID: node.uuid)
  }

  private func makeGroups(from nodes: [NodeInfo]) -> [NodeGroup] {
    Dictionary(grouping: nodes, by: { $0.region }).map { name, members in
      NodeGroup(id: UUID(), name: name, symbol: "folder", nodeIDs: members.map(\.id))
    }
  }
  ...
  private func makeMetrics(_ sample: KomariRealtime) -> NodeMetrics {
    ...
    return NodeMetrics(
      ...
      swapUsage: percentage(sample.swap.used, sample.swap.total),
      diskUsage: percentage(sample.disk.used, sample.disk.total),
      downloadBytesPerSecond: sample.network.down, uploadBytesPerSecond: sample.network.up,
      connectionCount: sample.connections.tcp + sample.connections.udp,
      processCount: sample.process, loadAverage: sample.load.load1, uptime: sample.uptime,
      diskReadBytesPerSecond: 0, diskWriteBytesPerSecond: 0)
  }

  private func makeHistory(_ records: [KomariHistoryRecord]) -> MetricsHistory {
    func samples(_ value: @escaping (KomariHistoryRecord) -> Double) -> [MetricSample] {
      records.map { MetricSample(id: UUID(), date: $0.time, value: value($0)) }
    }
    return MetricsHistory(
      cpu: samples(\.cpu), memory: samples { self.percentage($0.ram, $0.ramTotal) },
      download: samples { $0.netIn / 1_000_000 }, upload: samples { $0.netOut / 1_000_000 },
      diskRead: [], diskWrite: [])
  }

  private func percentage(_ used: Double, _ total: Double) -> Double {
...
private struct KomariNode: Decodable {
  let uuid: String
  let name: String
  let region: String?
  let group: String?
  ...
  enum CodingKeys: String, CodingKey {
    case uuid, name, region, group, os, tags
    case createdAt = "created_at"
  }
}
private struct KomariSocketEnvelope: Decodable { let data: KomariSocketPayload }
private struct KomariSocketPayload: Decodable {
  let online: [String]
  let data: [String: KomariRealtime]
}
private struct KomariRealtime: Decodable {
  let cpu: KomariCPU
  let ram: KomariMemory
  let swap: KomariMemory
  let load: KomariLoad
  let disk: KomariMemory
  let network: KomariNetwork
  let connections: KomariConnections
  let uptime: TimeInterval
  let process: Int
}
private struct KomariCPU: Decodable { let usage: Double }
private struct KomariMemory: Decodable {
  let total: Double
  let used: Double
}
private struct KomariLoad: Decodable { let load1: Double }
private struct KomariNetwork: Decodable {
  let up: Double
  let down: Double
}
private struct KomariConnections: Decodable {
  let tcp: Int
  let udp: Int
}
private struct KomariHistoryPayload: Decodable { let records: [KomariHistoryRecord] }
private struct KomariHistoryRecord: Decodable {
  let time: Date
  let cpu: Double
  let ram: Double
  let ramTotal: Double
...
extension JSONDecoder {
  fileprivate static var komari: JSONDecoder {
...
extension String {
  fileprivate var containsEmoji: String? {
    unicodeScalars.contains { $0.properties.isEmojiPresentation } ? self : nil
  }
}
